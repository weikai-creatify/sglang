...
class TestIndexerPlan(CustomTestCase):
        ...
        for ratios, topk, span, expected in (
            (
                ...
                [
                    ("candidate_all", 4),
                    ("candidate_c2_all", 8),
                    ("candidate_unfiltered", 16),
                ],
            ),
            ([2], 4, 16, [("candidate_all", 8), ("candidate_unfiltered", 16)]),
            ([1, 2], 4, 6, [("candidate_all", 4), ("candidate_c2_all", 6)]),
            ([1, 2], 4, 4, [("candidate_all", 4)]),
            ([4, 128], 4, 16, [("candidate_unfiltered", 16)]),
            ([1, 2], 0, 16, [("candidate_unfiltered", 16)]),
        ):
            with self.subTest(ratios=ratios, topk=topk, span=span):
                self.assertEqual(candidate_graph_limits(ratios, topk, span), expected)
    ...
    def test_publish_consume_and_bypass_are_distinct(self):
        ...
        self.assertIsNone(output)
        self.assertTrue(bypass.mask_topk)
    ...
    def test_paged_topk_dispatch_and_masked_output(self):
        ...
        for use_v2, capture_raw, mask in product((False, True), repeat=3):
            with self.subTest(use_v2=use_v2, capture_raw=capture_raw, mask=mask):
                ...
                with (
                    patch(
                        "sglang.srt.layers.attention.dsv4.indexer_topk.topk_transform_paged",
                        side_effect=v1,
                    ) as first,
                    patch(
                        "sglang.srt.layers.attention.dsv4.indexer_topk.topk_transform_paged_v2",
                        side_effect=v2,
                    ) as second,
                ):
                    write_paged_indexer_topk(
